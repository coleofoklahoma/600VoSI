...
public class MyGdxGame extends ApplicationAdapter {
    ...
    public void render() {
        ...
        //Move Camera
        if (Gdx.input.isKeyJustPressed(Keys.LEFT) && entityGrid.canMove("left")) {
            camera.position.x -= tileDimension;
            if (!bernard.isFlipX()) {
                bernard.flip(true, false);
            }
        } else if (Gdx.input.isKeyJustPressed(Keys.RIGHT) && entityGrid.canMove("right")) {
            camera.position.x += tileDimension;
            if (bernard.isFlipX()) {
                bernard.flip(true, false);
            }
        } else if (Gdx.input.isKeyJustPressed(Keys.UP) && entityGrid.canMove("up")) {
            camera.position.y += tileDimension;
        } else if (Gdx.input.isKeyJustPressed(Keys.DOWN) && entityGrid.canMove("down")) {
            camera.position.y -= tileDimension;
        }
